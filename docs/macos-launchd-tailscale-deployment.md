...
Initial throughput is exactly one active run: `service.max_concurrent_builds = 1`. A second eligible request receives immediate HTTP `503`, `Retry-After: 0`, and `{"error":"busy"}` before source persistence. There is no queue.
...
The lifecycle is synchronous. Client SIGINT or response disconnect closes the request and cancels the remote process group. Timeout and output exhaustion use the same bounded SIGTERM-then-SIGKILL escalation and require group disappearance before artifact collection and cleanup. Process groups are cleanup, not a sandbox against a deliberately escaped `setsid` process; account/VM isolation remains required for untrusted uploaded code.

Client run evidence remains private and non-destructive in its XDG result directory until client-side policy removes it. Server artifact archives remain until operator-selected `artifacts.ttl_sec` and/or `artifacts.max_bytes` GC bounds remove them; when both are unset, no finite server retention period is implied.
...
The harness builds the stable Nix server/client outputs, verifies that each package contains only its named binary, and executes those exact store binaries through a bounded local loopback flow. A fixed server-configured task reads uploaded source, streams distinct stdout/stderr markers, writes an allowlisted artifact derived from the upload, and exits 7. The harness verifies streamed-before-exit output, exact exit propagation, private result evidence and modes, provenance/manifest content, atomic artifact retrieval, and fresh-workspace cleanup.

This is package/protocol evidence, not a production security bypass or a native macOS runtime attestation. It uses the supported unauthenticated loopback configuration only inside an isolated unprivileged test and does not change production defaults or validation. The task is a trusted server-owned script (or may use executable/fixed-argv compatibility), not caller-provided shell authority. The harness uses no forge, SSH/remote-shell transport, source publication, Xcode, or remote Mac. It remains a Devenv/script check rather than a universal flake check because loopback networking is not portable across all Nix build sandboxes.
...
Linux behavioral tests and native Darwin package compilation are supporting evidence, not substitutes for this gate. The authoritative users, launchd module, Tailscale state, secrets, concrete tasks, and native acceptance results belong to deployment automation/operator policy.
